export default class GenericSerial {
  constructor() {
    this.filter_stlink = { usbVendorId: 0x483, usbProductId: 0x374e };
  ...
  async connect() {
    await this.port.open({ baudRate: 1843200 });
  ...
  async get_port() {
    return await navigator.serial.getPorts({ filters: [this.filter_stlink] });
  }

  async request_port() {
    return await navigator.serial.requestPort({
      filters: [this.filter_stlink],
    });
  }
